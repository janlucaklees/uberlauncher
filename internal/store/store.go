...
type Store struct {
	mu      sync.RWMutex
	bySkill map[string]map[string]types.EntryDTO
}

func New() *Store {
	return &Store{bySkill: make(map[string]map[string]types.EntryDTO)}
}

func (s *Store) Publish(entries []types.EntryDTO) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	for _, entry := range entries {
		if _, ok := s.bySkill[entry.SkillName]; !ok {
			s.bySkill[entry.SkillName] = make(map[string]types.EntryDTO)
		}
...
func (s *Store) Upsert(entries []types.EntryDTO) error {
...
func (s *Store) ListAll() []types.EntryDTO {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Store) Get(key EntryKey) (types.EntryDTO, bool) {
	s.mu.RLock()
	defer s.mu.RUnlock()

	bucket, ok := s.bySkill[key.SkillName]
	if !ok {
		return types.EntryDTO{}, false
	}
	entry, ok := bucket[key.EntryID]
	return entry, ok
}

func validateDuplicates(entries []types.EntryDTO) error {
